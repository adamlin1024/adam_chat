...
**所有功能新增、改動、修 bug 必須從第一性原理出發、循序漸進從根本解決問題。**

禁止只做單點的表象修正。動手前必須通盤評估檔案之間、功能之間的相依性，避免「改了 A 壞了 B」、避免最後才發現要還原走別條路、繞遠路。

以「資深系統架構師」的角度做嚴謹、系統性的排查 — 寧可前期評估多花時間，也不要後期狂修副作用。

### 動手前必跑的三步驟

1. **事前評估與分析**
   不要馬上動刀。先掃過、釐清這次改動可能影響的範圍，預判會不會引發其他 bug。
   - 找出所有 caller / callee
   - 找出共用的 state / cache / storage / context / SW / localStorage key
   - 確認既有功能流程與測試

2. **提出方案 + 風險評估**
   提出解法時**明確列出**改動的風險與影響範圍，從多個選項中選最佳。
   - 列出 2–3 個可行解法
   - 標註每個解法的風險、影響面、副作用
   - 挑最符合第一性原理（從根因切入）的那個，把選擇理由講清楚

3. **不繞遠路**
   不要只改表象、引發更多問題、最後才發現要還原走別條路。
   - 改之前先反問：這是症狀還是根因？有沒有更上游的點可以一次解掉？
   - 同類 bug 連續修兩次以上 = 根因找錯，停下來重新評估

### 實務檢查表（動手前自問）

- [ ] 我有完整理解現況、還是只看到症狀？
- [ ] 這個改動影響的檔案 / 功能 / state 都掃過了嗎？
- [ ] 有沒有更上游的根因可以一次解掉、避免 patch 越疊越多？
- [ ] 有沒有歷史教訓提醒我這條路走不通？（看 memory、git log、CLAUDE.md）
