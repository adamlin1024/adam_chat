...
const Type: FC<Props> = ({ select = "", updateFilter }) => {
  ...
  return (
    <div className="p-3 bg-bg-elevated min-w-[180px] overflow-auto shadow-md rounded-lg flex flex-col items-start relative">
      <ul className="w-full flex flex-col gap-4">
        <li
          className="relative cursor-pointer flex items-center gap-4 text-fg-body font-semibold text-sm"
        ...
        {Object.entries(FileTypes).map(([type, { title, icon }]) => {
          return (
            <li
              key={title}
              className="relative cursor-pointer flex items-center gap-2 text-sm text-fg-body font-semibold"
