...
const RegWithUsername: FC = () => {
  ...
  useEffect(() => {
    if (loginError && "status" in loginError) {
      switch (loginError.status) {
        case 401:
          toast.error(t("tip.invalid_token", { ns: "common" }));
          break;
        default:
          break;
      }
  ...
  useEffect(() => {
    if (regError && "status" in regError) {
      switch (regError.status) {
        case 409:
          toast.error(t("tip.something_conflict", { ns: "common" }));
          break;
        default:
          break;
      }
  ...
  if (!token) return <span className="text-fg-primary">No Token</span>;
  const isLoading = loginLoading || regLoading;
  const isSuccess = loginSuccess || regSuccess;
  return (
    <>
      <div className="flex-center flex-col pb-6 max-w-md">
        <h2 className="font-semibold text-2xl text-fg-primary mb-2">
          {t("reg.input_name")}
        </h2>
        <span className="text-fg-secondary text-fg-primary">
