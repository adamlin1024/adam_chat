...
const LoginTip = ({ placement = "chat" }: Props) => {
  ...
  return (
    <div
      className={clsx(
        "flex items-center justify-between bg-bg-elevated rounded-lg p-4 border border-solid border-border-strong",
        placement == "session"
          ? "!w-[96%] md:hidden fixed bottom-2 left-1/2 -translate-x-1/2"
          : "w-full"
      )}
    >
      <span className="text-xs md:text-base text-fg-primary">
