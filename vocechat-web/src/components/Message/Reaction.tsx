...
const Reaction: FC<Props> = ({ mid, reactions = null, readOnly = false }) => {
  ...
  return (
    <span className="group relative mt-1.5 flex items-center gap-1 w-fit">
      {Object.entries(reactions).map(([reaction, uids], idx) => {
        ...
        return uids.length > 0 ? (
          <span
            ...
            className={`cursor-pointer rounded-sm relative flex items-center gap-1 px-2.5 py-[3px] border transition-colors ${
              reacted
      ...
      {!readOnly && (
        <Tooltip placement="top" tip="Add Reaction">
          ...
          >
            <button className="invisible group-hover:visible w-6 h-6 border border-border bg-bg-surface hover:border-border-strong rounded-sm flex-center transition-colors">
              <IconAddEmoji className={"w-3.5 h-3.5 fill-fg-subtle"} />
            </button>
          </Tippy>
        </Tooltip>
      )}
