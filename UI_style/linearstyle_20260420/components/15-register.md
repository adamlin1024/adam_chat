...
<div class="flex min-h-screen items-center justify-center bg-bg-app">
  <form class="flex w-[320px] flex-col items-center gap-3.5 rounded-[10px]
    ...
    <h1 class="text-[16px] font-bold text-fg-primary">建立帳號</h1>

    {status === 'valid' && (
      <p class="font-mono text-[10.5px] text-green-400">✓ 邀請連結有效</p>
    )}
    {status === 'expired' && (
      <p class="font-mono text-[10.5px] text-red-400">✗ 邀請連結已過期</p>
    )}

    <input placeholder="你的名稱" />
    <input type="password" placeholder="設定密碼" />

    <button disabled={status !== 'valid'}
      class="w-full rounded-md bg-accent-300 py-2.5
