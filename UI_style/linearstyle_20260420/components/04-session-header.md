...
- 高度：由內容決定（padding 14/16/12）
- 上方有一列 mono 小字：`WORKSPACE`（`color: #3f3f46`，`font-mono 9px`，`letter-spacing: 0.08em`）
- 組織名稱：`Inter 600 / 13px`，`color: #f4f4f5`，`letter-spacing: -0.01em`
- 右側 `+` 按鈕：22×22px，`border: 1px solid #27272a`，`rounded-md`，`color: #71717a`
...
<header class="flex items-center justify-between border-b border-border-subtle
               px-4 pt-3.5 pb-3">
  <div>
    <div class="font-mono text-[9px] tracking-[0.08em] text-fg-disabled">WORKSPACE</div>
    <div class="text-[13px] font-semibold tracking-tight text-fg-primary">Adam_chat</div>
  </div>
  {isAdmin && (
    <button class="flex h-[22px] w-[22px] items-center justify-center
