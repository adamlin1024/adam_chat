...
容器：
- padding：`7px 10px`，margin：`1px 8px`（橫向縮進）
- `border-radius: 6px`
- gap 10px

內容：
- Avatar（[2-3]）：24×24px — channel: rounded-md / user: rounded-full
- Name：`Inter 500 / 12.5px`，`color: #e4e4e7`，truncate
- Preview：`Inter 400 / 10.5px`，`color: #52525b`，truncate
- Badge（未讀）：`font-mono 600 / 9.5px`，`bg: #5eead4`，`color: #042f2e`，padding `1px 6px`，`rounded-sm`
...
- 樣式：`font-mono 9px / 600`，`color: #3f3f46`，`letter-spacing: 0.14em`，uppercase
...
<button data-active="true"
  class="mx-2 my-px flex items-center gap-2.5 rounded-md px-2.5 py-[7px]
         hover:bg-[#0f1014]
  ...
  <div class="min-w-0 flex-1">
    <div class="truncate text-[12.5px] font-medium text-[#e4e4e7]">general</div>
    <div class="truncate text-[10.5px] text-fg-subtle mt-px">Adam: 這個介面我想改</div>
  </div>
  {unreadCount && (
    <span class="rounded-sm bg-accent-300 px-1.5 py-px
                 font-mono text-[9.5px] font-bold text-teal-950">3</span>
  )}
</button>
```
