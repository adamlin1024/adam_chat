...
- `登入 Adam_chat` — `Inter 700 / 20px`，`color: #f4f4f5`，`letter-spacing: -0.015em`

### Subtitle

- `font-mono 12px`，`color: #71717a`
- 例：`輸入帳號密碼繼續`
...
- `rounded-md`, `font-mono 13px / 700 / 0.1em caps`
...
<div class="flex min-h-screen items-center justify-center bg-bg-app">
  <form class="flex w-[360px] flex-col items-center gap-3.5 rounded-[10px]
    ...
    <div class="flex h-12 w-12 items-center justify-center rounded-md
                bg-accent-300 text-[20px] font-extrabold text-teal-950">A</div>
    <h1 class="text-[20px] font-bold tracking-tight text-fg-primary">登入 Adam_chat</h1>
    <p class="font-mono text-[12px] text-fg-muted">輸入帳號密碼繼續</p>

    <input class="w-full rounded-md border border-border bg-bg-app px-3.5 py-[11px]
    ...
    <button class="w-full rounded-md bg-accent-300 py-[11px]
                   font-mono text-[13px] font-bold tracking-wider text-teal-950">
      SIGN IN
    </button>
    <button class="font-mono text-[12px] text-fg-muted hover:text-fg-secondary">
      或使用 Magic Link 登入
    </button>
  </form>
</div>
