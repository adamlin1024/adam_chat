...
1. **Banner 區**：高度 56px，`background: #141519`（**不使用漸層**）
2. **Avatar 區**：
   - Avatar 2xl (64px)，margin-top -32px，`border: 3px solid #0c0d10` 切開 banner
   - padding 左 14px
3. **資料區** (padding `0 14px 14px`)：
   - Name：`Inter 700 / 16px`，`color: #f4f4f5`，mt 8px
   - Status：`font-mono 12px`，`color: #71717a`，例：`管理員 · 在線`
...
4. **操作區**：
   - 主按鈕「傳送訊息」：
     ...
     - `rounded-md`
   - admin 額外選項：「修改暱稱」，以純文字按鈕呈現 `font-mono 12px / #71717a`
...
<div class="w-[220px] overflow-hidden rounded-[10px] border border-border
  ...
  <div class="h-14 bg-bg-surface"></div>
  <div class="px-3.5 pb-3.5">
    <Avatar size="2xl" class="-mt-8 ring-[3px] ring-bg-elevated" />
    <div class="mt-2 text-[16px] font-bold text-fg-primary">Adam</div>
    <div class="font-mono text-[12px] text-fg-muted">
      管理員 · <span class="text-green-400">● 在線</span>
    </div>
    <button class="mt-2.5 w-full rounded-md bg-accent-300 py-2.5
                   text-[14px] font-bold text-teal-950">
      傳送訊息
    </button>
    {isAdmin && (
      <button class="mt-2 w-full font-mono text-[12px] text-fg-muted hover:text-fg-secondary">
        修改暱稱
      </button>
    )}
  </div>
